...
def add_gym(request, user_id):
    if request.method == 'POST':
        ...
        spraywall_instance = create_spraywall(spraywall_data, gym_instance.id)
        update_person_data(user_id, gym_instance, spraywall_instance)
        person = Person.objects.get(id=user_id)
        data = {
            'gym': get_auth_gym_data(person),
            'spraywalls': get_spraywalls(gym_instance.id),
        }
...
def query_gyms(request):
    if request.method == 'GET':
        ...
        gyms = Gym.objects.filter(Q(name__icontains=search_query) | Q(
...
def edit_gym(request, gym_id):
    if request.method == 'POST':
        ...
        gym_serializer = GymSerializer(
            instance=gym, data=request.data, partial=True)
        if gym_serializer.is_valid():
            # Save the gym instance and get the saved object
            gym_instance = gym_serializer.save()
            data = {
